    ...
    pet_products = [
        {
            ...
            "price": 58000,
            "category": "사료",
            "tags": ["대형견", "노령견", "관절건강", "프리미엄사료", "처방보조식", "시니어"],
            "description": """
        ...
        },
        {
            ...
            "price": 18000,
            "category": "장난감",
            "tags": ["강아지", "장난감", "삑삑이", "스트레스해소", "내구성", "중형견", "인기상품"],
            "description": """
        ...
        },
        {
            ...
            "price": 25000,
            "category": "장난감",
            "tags": ["강아지", "고양이", "장난감", "자동", "레이저", "활동량증가", "실내놀이"],
            "description": """
        ...
        },
        {
            ...
            "price": 92000,
            "category": "가구",
            "tags": ["고양이", "스크래처", "타워", "캣타워", "휴식공간", "놀이터", "인기상품"],
            "description": """
...
def initialize_vectorstore():
    ...
    try:
        ...
        persist_directory_str = str(settings.CHROMA_DB_PATH) if settings.CHROMA_DB_PATH else None
        ...
        if persist_directory_str:
            ...
            logger.warning("ChromaDB 저장 경로가 설정되지 않았습니다. 인메모리로 실행될 수 있습니다.")

        # 벡터 스토어 가져오기 또는 생성
        # get_or_create_vectorstore 함수가 내부적으로 임베딩 모델을 처리합니다.
        logger.info(f"벡터 스토어 가져오기/생성 시도: collection='{collection_name}', directory='{persist_directory_str}'")
        vectorstore = get_or_create_vectorstore(
             collection_name=collection_name,
             persist_directory=persist_directory_str
        )

        if not isinstance(vectorstore, Chroma): # 또는 VectorStore (langchain_core.vectorstores.VectorStore)
             logger.error(f"get_or_create_vectorstore가 Chroma (또는 VectorStore) 인스턴스를 반환하지 않았습니다 (반환 타입: {type(vectorstore)}). 중단합니다.")
             return False
        
        # get_or_create_vectorstore 함수 내부에서 사용된 임베딩 모델 확인 (디버깅용)
        if hasattr(vectorstore, 'embedding_function') and vectorstore.embedding_function:
            logger.info(f"벡터 스토어에 사용된 임베딩 모델: {type(vectorstore.embedding_function)}")
        elif hasattr(vectorstore, '_embedding_function') and vectorstore._embedding_function: # 일부 래퍼는 _ 언더스코어 사용
             logger.info(f"벡터 스토어에 사용된 임베딩 모델 (내부): {type(vectorstore._embedding_function)}")
        else:
            logger.warning("벡터 스토어에서 사용 중인 임베딩 함수 정보를 직접 확인할 수 없습니다 (get_or_create_vectorstore 내부 로직 확인 필요).")
        ...
        try:
            ...
            logger.warning(f"기존 문서 수 확인 중 오류: {e}. 컬렉션이 아직 없거나 접근 불가일 수 있습니다.")
        
        # 데이터 삭제 및 추가 로직
        should_load_new_data = True
        if current_doc_count > 0:
            logger.info(f"이미 벡터 스토어에 {current_doc_count}개의 문서가 있습니다.")
            response = input(f"기존 데이터({current_doc_count}개)를 삭제하고 새로 로드하시겠습니까? (y/n, 기본값 n): ").lower()
            ...
            if response == 'y':
                ...
                try:
                    ...
                    logger.error(f"기존 데이터 삭제 중 오류 발생: {e_delete}", exc_info=True)
        ...
        if should_load_new_data:
            ...
            split_docs = split_documents(documents_to_load, chunk_size=1000, chunk_overlap=200)
            ...
            if persist_directory_str:
                try:
                    if hasattr(vectorstore, 'persist') and callable(vectorstore.persist):
                        vectorstore.persist()
                        logger.info("벡터 스토어 변경사항을 디스크에 저장했습니다 (vectorstore.persist()).")
                    elif hasattr(vectorstore._client, 'persist') and callable(vectorstore._client.persist):
                        vectorstore._client.persist()
                        logger.info("벡터 스토어 변경사항을 디스크에 저장했습니다 (vectorstore._client.persist()).")
                    else:
                        logger.info("사용 중인 Chroma 버전에서 persist() 메소드를 찾을 수 없습니다. 자동 저장될 수 있습니다.")
                except Exception as e_persist:
                    logger.error(f"데이터 persist 중 오류 발생: {e_persist}", exc_info=True)

            final_doc_count = vectorstore._collection.count()
            logger.info(f"벡터 스토어 최종 문서 수: {final_doc_count}")
            
            if final_doc_count < len(split_docs) and (current_doc_count == 0 or response == 'y'):
                logger.warning(f"주의: 최종 로드된 문서 수({final_doc_count})가 예상된 청크 수({len(split_docs)})보다 적습니다.")
        ...
        logger.error(f"벡터 스토어 초기화 중 예측하지 못한 오류 발생: {str(e)}", exc_info=True)
        return False
...
if __name__ == "__main__":
    ...
    if success:
        logger.info(f"✅ 벡터 스토어 초기화/데이터 로드 작업 성공적으로 완료! (소요 시간: {duration:.2f}초)")
    else:
        logger.error(f"❌ 벡터 스토어 초기화/데이터 로드 작업 실패. (소요 시간: {duration:.2f}초)")
    logger.info("="*50)