...
const shop = {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.success) {
                    ...
                    if (!isLoggedIn) {
                        if (confirm(response.message || '로그인이 필요한 서비스입니다. 로그인 페이지로 이동하시겠습니까?')) {
    ...
    initializeLikeButtons: () => {
        ...
        const likeButtons = document.querySelectorAll('.product__hover .like-button');
        ...
        likeButtons.forEach(button => {
            ...
            if (isLoggedIn) {
                console.log(`[${itemKey}] Checking initial liked status (isLoggedIn: ${isLoggedIn}).`);
                ...
                console.log(`[${itemKey}] Skipping initial liked status check (isLoggedIn: ${isLoggedIn}).`);
            }
...
let cartAddUrl, likeToggleUrl, likeCheckUrl, loginUrl, keyword;

$(function () {
    cartAddUrl = document.body.dataset.cartAddUrl || '';
    ...
    if (shopPageData.length > 0) {
        ...
        $('#product-list-container').on('click', '.add-cart', function (e) {
            console.log("Add to cart listener fired!"); // 로그 확인
            e.preventDefault();
            const itemKey = $(this).data('item-key');
            if (itemKey) {
                shop.addToCart(itemKey);
            } else {
                console.error("Item key not found for add to cart button.");
            }
        });
