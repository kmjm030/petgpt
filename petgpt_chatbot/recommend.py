...
# 동물 종류 키워드
PET_TYPES = {
    "강아지": ["강아지", "개", "puppy", "dog", "멍멍이", "댕댕이", "애견", "멍뭉이", "반려견"],
    "고양이": ["고양이", "cat", "kitten", "야옹이", "냥이", "애묘", "냥냥이", "반려묘", "kitty"]
}
...
PRODUCT_CATEGORIES = {
    ...
    "위생용품": ["샴푸", "shampoo", "목욕", "bath", "청소", "화장실", "배변", "위생", "hygiene"],
    "영양제": ["영양제", "supplement"],
    "스크래처": ["스크래처", "스크래쳐"],
    "건강관리": ["건강", "health", "관절", "눈", "눈물", "피부", "모질", "노령화", "치아"]
}

# 문제/증상 키워드와 관련 상품 특성 매핑
SYMPTOM_FEATURES = {
    "관절": ["관절", "뼈", "골격", "걷기", "통증", "절뚝", "절름발이", "오르기", "내리기", "관절염"],
    "피부": ["피부", "가려움", "털빠짐", "모질", "비듬", "습진", "발진", "홍반", "피부병", "건조"],
    "위장": ["설사", "구토", "소화", "위장", "배탈", "소화불량", "복통", "식욕부진", "위염", "장염"],
...
# 생애주기 키워드
LIFE_STAGES = {
    "퍼피/키튼": ["퍼피", "강아지", "어린", "새끼", "태어난", "젊은", "키튼", "young", "baby", "puppy", "kitten"],
    "어덜트": ["성묘", "성견", "어른", "성인", "중년", "adult", "성체"],
    "시니어": ["노령", "노인", "늙은", "오래된", "고령", "시니어", "중년", "노견", "노묘", "aged", "old", "senior", "elderly"]
}
...
def parse_product_request_rules(query: str) -> Dict[str, Any]:
    ...
    sorted_categories = sorted(PRODUCT_CATEGORIES.items(), key=lambda item: max(len(k) for k in item[1]), reverse=True)
    ...
    generic_filter = {
        "추천", "해줘", "좀", "우리", "저희", "요", "해주세요", "부탁해요", "찾고", "있어요", "입니다", "인데", "에서", "에", "의", "이가", "는", "은", "이", "가",
    ...
    for kw_list in PET_TYPES.values(): generic_filter.update(kw_list)
    for cat_name in PRODUCT_CATEGORIES.keys(): generic_filter.add(cat_name.lower()) # 카테고리 대표명 필터 추가
    ...
    for word in candidate_words:
        ...
        if clean_word and len(clean_word) > 1 and clean_word not in generic_filter and clean_word not in animal_type_keywords_used and clean_word not in category_keywords_used:
...
def extract_product_details_llm(query: str, parsed_info: Dict[str, Any]) -> Dict[str, Any]:
    """
    ...
    needs_llm_enhancement = (
        parsed_info.get('animal_type') is None or
        parsed_info.get('category') is None or
        len(parsed_info.get('keywords', [])) < 2 # 키워드가 2개 미만일 때 LLM 도움
    )
    ...
    if needs_llm_enhancement:
        try:
            ...
            prompt_input = {
                ...
                "keywords": ", ".join(parsed_info.get('keywords', [])) if parsed_info.get('keywords') else "파악되지 않음"
            }
            # prompt = PRODUCT_DETAIL_EXTRACTION_PROMPT_LLM.format(**prompt_input) # 직접 format 호출 대신 chain으로
            
            # LLM 호출 및 결과 파싱
            output_parser = StrOutputParser()
            chain = PRODUCT_DETAIL_EXTRACTION_PROMPT_LLM | llm | output_parser # 프롬프트 템플릿을 먼저 연결
            ...
            try:
                ...
                if json_match:
                    ...
                    if start_idx >= 0 and end_idx >= 0:
                        ...
                        # JSON 형식이 아닌 경우
                        llm_data = {"error": "JSON 형식으로 파싱할 수 없는 응답", "original_response": llm_response_str}
                ...
                if isinstance(llm_data, dict) and "error" not in llm_data:
                    ...
                    if enhanced_info.get('animal_type') is None and llm_data.get('animal_type'):
                        animal_type_from_llm = str(llm_data['animal_type']).lower()
                        if '고양이' in animal_type_from_llm or 'cat' in animal_type_from_llm:
                            enhanced_info['animal_type'] = '고양이'
                        elif '강아지' in animal_type_from_llm or 'dog' in animal_type_from_llm:
                            enhanced_info['animal_type'] = '강아지'
                    
                    # 카테고리가 없는 경우 LLM 추론 결과로 보강 (parsed_info 우선)
                    if enhanced_info.get('category') is None and llm_data.get('category'):
                        category_from_llm = str(llm_data['category']).lower()
                        for cat_name_key, cat_keywords_val in PRODUCT_CATEGORIES.items():
                            if category_from_llm == cat_name_key.lower() or category_from_llm in [kw.lower() for kw in cat_keywords_val]:
                                enhanced_info['category'] = cat_name_key
                                break

                    # 키워드가 부족한 경우 LLM 결과로 보강 (기존 키워드에 추가)
                    if llm_data.get('keywords') and isinstance(llm_data['keywords'], list):
                    ...
                    for key in ["pet_characteristics", "price_range", "brand_preference", "priorities", "additional_info"]:
                        if key in llm_data:
                            enhanced_info['llm_extracted'][key] = llm_data[key]
                elif "error" in llm_data:
                    enhanced_info['llm_error'] = f"LLM 응답 JSON 파싱 오류: {llm_data['error']}, 원본: {llm_data.get('original_response', '')}"
            
            except json.JSONDecodeError as e:
                # JSON 파싱 실패 시 원래 정보 유지
                enhanced_info['llm_error'] = f"LLM 응답 JSON 디코딩 오류: {str(e)}, 원본: {llm_response_str}"
...
def search_products_in_db(parsed_info: Dict[str, Any], limit: int = 10) -> List[Dict[str, Any]]:
    """
    ...
    try:
        ...
        if 'llm_extracted' in parsed_info and isinstance(parsed_info['llm_extracted'], dict):
            # 브랜드 선호도
            ...
            if 'price_range' in parsed_info['llm_extracted']:
                ...
                if isinstance(price_range, str):
                    ...
                    price_match = re.search(r'(\d+)[만원]*[~\-](\d+)[만원]*', price_range)
                    if price_match:
                        min_price = int(price_match.group(1)) * 10000 if '만' in price_range else int(price_match.group(1))
                        max_price = int(price_match.group(2)) * 10000 if '만' in price_range else int(price_match.group(2))
                        ...
                        if price_match:
                            max_price = int(price_match.group(1)) * 10000 if '만' in price_range else int(price_match.group(1))
...
def select_final_products_llm(query: str, products: List[Dict[str, Any]], max_products: int = 3) -> List[Dict[str, Any]]:
    """
    ...
    try:
        ...
        for i, product in enumerate(products):
            ...
            # 상품 설명 요약 (너무 길면 자름)
            content_summary = item_content[:200] + '...' if len(item_content) > 200 else item_content
        ...
        if id_match:
            ...
            selected_products = [p for p in products if p.get('item_key') in selected_ids]
            ...
            return sorted(products, key=lambda x: x.get('sales_count', 0), reverse=True)[:max_products]
...
def generate_recommendation_message_llm(query: str, products: List[Dict[str, Any]]) -> str:
    """
        ...
        try:
            product_info_list.append(
                ProductInfo(
                    ...
                    price=sale_price,
                    rating=product.get("rating", 4.5),  # 평점 정보가 없으면 기본값 4.5 사용
                    image_url=image_url,
