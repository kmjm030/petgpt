...
def get_mysql_connection():
    ...
    if not MYSQL_AVAILABLE:
        raise ImportError(
            "mysql.connector 패키지가 설치되어 있지 않습니다. 'pip install mysql-connector-python'을 실행하세요."
        )

    settings = get_settings()
    print(
        f"MySQL 연결 시도: {settings.MYSQL_HOST}:{settings.MYSQL_PORT}, DB={settings.MYSQL_DB_NAME}, User={settings.MYSQL_USER}"
    )
...
def execute_mysql_query(
    query: str, params: Any = None, fetch: bool = True
) -> List[Dict[str, Any]]:
    """
    ...
    settings = get_settings()
    print(
        f"MySQL 연결 정보: Host={settings.MYSQL_HOST}, User={settings.MYSQL_USER}, DB={settings.MYSQL_DB_NAME}, Port={settings.MYSQL_PORT}"
    )
...
def log_conversation(log_data: Dict[str, Any]) -> None:
    ...
    if hasattr(raw_bot_response, "model_dump_json"):  # Pydantic 모델인 경우
        ...
        raw_bot_response, "model_dump"
    ):  # Pydantic v2 model_dump(mode='json') 대체
        bot_response_str = json.dumps(
            raw_bot_response.model_dump(mode="json"), ensure_ascii=False
        )
        ...
        bot_response_str = str(
            raw_bot_response
        )  # 이미 문자열이거나 다른 타입일 경우 대비

    intent = log_data.get(
        "response_type",
        (
            raw_bot_response.get("response_type", "general")
            if isinstance(raw_bot_response, dict)
            else "general"
        ),
    )
