...
class SpringChatResponse(BaseModel):
    ...
    responseType: str = Field(
        ..., description="응답 유형 (general, qna, product_recommendation)"
    )
    products: Optional[List[SpringProductInfo]] = Field(
        None, description="추천 상품 목록"
    )
    ...
    try:
        ...
        try:
            ...
            if "qna" in intent_line:
                ...
                "product_recommendation" in intent_line
                or "recommendation" in intent_line
            ):
...
async def handle_general_conversation(query: str) -> ChatResponse:
    ...
    # 인사말이나 감사 표현과 같은 일반적인 대화
        ...
        word in query_lower
        for word in ["안녕", "반가", "고마", "감사", "잘 지내", "ㅋㅋ", "ㅎㅎ"]
    ):
        return ChatResponse(
            response_type="general",
...
async def chat(request: ChatRequest) -> ChatResponse:
    ...
    try:
        logger.debug(
            f"채팅 API 호출: query='{request.query}', session_id='{request.session_id}'"
        )
        ...
        if intent == "qna":
            try:
                ...
                logger.debug(
                    f"QnA 파이프라인 실행 완료: answer_length={len(answer)}, sources={sources}"
                )
            ...
            try:
                ...
                logger.debug(
                    f"상품 추천 파이프라인 실행 완료: message_length={len(result['message'])}, products_count={len(result.get('products', []))}"
                )
                ...
                if not result.get("products"):
                    ...
                    response = ChatResponse(
                        ...
                        response_text=result.get(
                            "message",
                            "죄송합니다. 요청하신 조건에 맞는 상품을 찾을 수 없습니다. 다른 조건으로 검색해 보시겠어요?",
                        ),
                    )
                else:
                    response = ChatResponse(
...
async def chatbot_ask(request: SpringChatRequest) -> SpringChatResponse:
    ...
    try:
        ...
        if response.products:
            spring_products = [
                convert_to_spring_product(product) for product in response.products
            ]
