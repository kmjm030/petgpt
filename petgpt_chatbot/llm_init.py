...
class MockChatModel(BaseChatModel):
    ...
    def _call(self, messages, stop=None, run_manager=None, **kwargs):
        ...
        return AIMessage(content="죄송합니다. 현재 시스템 점검 중입니다. 잠시 후 다시 시도해 주세요.")
    ...
    async def _agenerate(self, messages, stop=None, run_manager=None, **kwargs):
        ...
        response = ChatResult(
            generations=[
                ChatGeneration(
                    message=AIMessage(content="죄송합니다. 현재 시스템 점검 중입니다. 잠시 후 다시 시도해 주세요.")
                )
            ]
        )
        return response
...
def get_llm() -> BaseChatModel:
    ...
    try:
        ...
        # 모델명 확인 및 설정
        model_name = settings.GEMINI_MODEL_NAME if settings.GEMINI_MODEL_NAME else GEMINI_MODEL_NAME
        logger.debug(f"LLM 초기화 시작: 모델={model_name}")
        
        # 최대 토큰 설정
        max_output_tokens = int(settings.MAX_TOKENS_PER_RESPONSE) if settings.MAX_TOKENS_PER_RESPONSE else 512
...
def get_embedding_model() -> Union[GoogleGenerativeAIEmbeddings, SentenceTransformerEmbeddings]:
    ...
    try:
        # 사용자 지정 임베딩 모델 타입 확인
        embedding_model_type = settings.EMBEDDING_MODEL_TYPE.lower() if settings.EMBEDDING_MODEL_TYPE else "sbert"
        
        if embedding_model_type == "gemini":
            logger.debug(f"Gemini 임베딩 모델 초기화 시작: 모델={GEMINI_EMBEDDING_MODEL_NAME}")
            ...
            logger.debug(f"Gemini 임베딩 모델 테스트 성공: 벡터 차원={len(test_embedding)}")
            logger.debug("Gemini 임베딩 모델 초기화 완료")
        else:
            logger.debug(f"SentenceTransformer 임베딩 모델 초기화 시작: 모델={settings.SBERT_MODEL_NAME}")
            # Sentence-Transformers 모델 사용 (한국어 지원 모델)
            model_name = settings.SBERT_MODEL_NAME if settings.SBERT_MODEL_NAME else "jhgan/ko-sroberta-multitask"
        ...
        logger.warning("임베딩 모델 초기화 실패로 기본 SentenceTransformer 모델을 사용합니다.")
