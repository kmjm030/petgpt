...
def retrieve_relevant_documents(
    query: str, vectorstore: VectorStore, k: int = 4
) -> List[Document]:
    """
    쿼리와 관련성 높은 문서를 검색
...
def get_or_create_vectorstore(
    collection_name: Optional[str] = None, persist_directory: Optional[str] = None
) -> VectorStore:
    """
...
def create_rag_pipeline(vectorstore: Optional[VectorStore] = None):
    ...
    def log_and_retrieve_docs(
        query_input,
    ):  # retriever는 이 함수 스코프 외부에서 정의됨
        ...
        for i, doc_retrieved in enumerate(retrieved_docs):
            print(
                f"DEBUG RAG: Doc {i+1} Title: {doc_retrieved.metadata.get('title', 'N/A')}, Source: {doc_retrieved.metadata.get('source', 'N/A')}, Type: {doc_retrieved.metadata.get('type', 'N/A')}"
            )
    ...
    rag_chain = (
        RunnableParallel(
            ...
            | RunnableLambda(
                log_and_retrieve_docs
            ),  # 사용자 질문(str)을 받아 문서 검색 및 로깅
        )
        | RunnableParallel(
        ...
        | RunnableParallel(
            ...
            has_medical_content=RunnableLambda(
                lambda x: x["has_medical_content"] or check_medical_content(x["query"])
            ),
        )
        | RunnableParallel(
            raw_answer=RunnableLambda(
                lambda x: {"context": x["context"], "query": x["query"]}
            )
            | QNA_RAG_PROMPT
            | llm
            | StrOutputParser(),
        ...
        | RunnableParallel(
            answer=RunnableLambda(
                lambda x: format_rag_response(
                    x["raw_answer"], x["sources"], x["has_medical_content"]
                )
            ),
            sources=RunnableLambda(lambda x: x["sources"]),
        )
...
def get_qna_answer(
    query: str, vectorstore: Optional[VectorStore] = None
) -> Tuple[str, List[str]]:
    """
    ...
    is_food_related = bool(
        re.search(
            r"food|feed|사료|먹이|식사|식품|간식|영양|nutrition|diet|dietary",
            query.lower(),
        )
    )
