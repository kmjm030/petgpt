...
TEST_QUERIES = [
    ...
    # 상품 추천 의도 쿼리
    {
        "query": "Can you recommend dog food for a puppy?",
        "description": "상품 추천 (영어)",
    },
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return (
                False,
                response_time,
                f"오류 응답: {response.status_code} - {response.text}",
            )
...
def main():
    ...
    """
    print(
        f"PetGPT 챗봇 API 응답 시간 측정 시작 ({datetime.now().strftime('%Y-%m-%d %H:%M:%S')})"
    )
    ...
    for query_data in TEST_QUERIES:
        ...
        if success_count > 0:
            ...
            print(
                f"  결과: 평균 {avg_time:.2f}초 (최소: {min_time:.2f}초, 최대: {max_time:.2f}초)"
            )
            print(
                f"  성공률: {success_count}/{ITERATIONS} ({(success_count / ITERATIONS) * 100:.0f}%)"
            )
        else:
            print("  모든 요청이 실패했습니다.")
